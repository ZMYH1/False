...
public class ESP extends Module {
    ...
    public final ListValue modeValue = new ListValue("Mode", new String[]{"Box", "FuckingYou", "OtherBox", "WireFrame", "2D", "Real2D", "Outline", "ShaderOutline", "ShaderGlow","CSGO"}, "Box");
    ...
    private final FloatValue CSGOWidth = new FloatValue("CSGO-Width", 2f, 0.5f, 5f);
            ...
            if (entity != null && entity != mc.thePlayer && EntityUtils.isSelected(entity, false)) {
                ...
                switch (mode.toLowerCase()) {
                    ...
                    case "csgo": {
                        final RenderManager renderManager = mc.getRenderManager();
                        final Timer timer = mc.timer;
                        final AxisAlignedBB bb = entityLiving.getEntityBoundingBox()
                        ...
                        for (double[] boxVertex: boxVertices){
                            final Vector2f screenPos = WorldToScreen.worldToScreen(new Vector3f((float) boxVertex[0],(float) boxVertex[1],(float) boxVertex[2])
                                    ,mvMatrix
                                    ,projectionMatrix
                                    ,mc.displayWidth
                                    ,mc.displayHeight);
                        ...
                        if (!(minX == (float) mc.displayWidth || minY == (float) mc.displayHeight || maxX == 0f || maxY == 0f)) {
