...
//            new MineplexGround(),
    ...
    public final BoolValue autodis = new BoolValue("AutoDisable",false);
    public final BoolValue blockS08 = new BoolValue("BlockS08",false);
    public final BoolValue debug = new BoolValue("Debug",false);

    LinkedList<S08PacketPlayerPosLook> packetBus = new LinkedList<>();

    @EventTarget
    public void onPacket(final PacketEvent event){
        final Packet<?> packet = event.getPacket();

        if (blockS08.get() && packet instanceof S08PacketPlayerPosLook){
            final S08PacketPlayerPosLook s08PacketPlayerPosLook = (S08PacketPlayerPosLook) packet;
            packetBus.add(s08PacketPlayerPosLook);
            event.cancelEvent();
            mc.thePlayer.sendQueue.addToSendQueue(new S08PacketPlayerPosLook());
        }

        if (event.getPacket() instanceof S08PacketPlayerPosLook && autodis.get()){
