...
class Breadcrumbs : Module() {
    ...
    private val speedonly = BoolValue("OnlySpeed", false)
    private val particley = FloatValue("Y",-0.5f,-2f,2f)
    private val mode = ListValue("Mode", arrayOf("Normal","Particle"),"Normal")
    private val particlemode = IntegerValue("ParticleMode",10,0,41)
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        if (speedonly.get() && LiquidBounce.moduleManager.getModule(Speed::class.java)!!.state || !speedonly.get()) {
            synchronized(points) {
                GL11.glPushMatrix()
