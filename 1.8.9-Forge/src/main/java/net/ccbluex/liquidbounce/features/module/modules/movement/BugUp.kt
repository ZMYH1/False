...
class BugUp : Module() {
    private val modeValue = ListValue("Mode", arrayOf("TeleportBack", "FlyFlag", "OnGroundSpoof", "MotionTeleport-Flag","Minemora"), "FlyFlag")
        ...
        if (!mc.thePlayer.onGround && !mc.thePlayer.isOnLadder && !mc.thePlayer.isInWater) {
            ...
            if (mc.thePlayer.fallDistance - lastFound > maxDistanceWithoutGround.get()) {
                ...
                when (mode.toLowerCase()) {
                    ...
                    "minemora" -> {
                        mc.netHandler.addToSendQueue(C03PacketPlayer.C04PacketPlayerPosition(prevX,prevY,prevZ,true))
                        mc.netHandler.addToSendQueue(C03PacketPlayer.C04PacketPlayerPosition(prevX,prevY,prevZ,true))
                    }
                    "flyflag" -> {
