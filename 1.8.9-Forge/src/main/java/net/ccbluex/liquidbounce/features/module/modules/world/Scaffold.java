...
public class Scaffold extends Module {
    ...
    // Rotations
    private final ListValue rotationsValue = new ListValue("Rotations", new String[] {"Liquidbounce","Custom"},"Liquidbounce");
    private final IntegerValue maxcustomYaw = new IntegerValue("MaxCustomRotationYaw",180,-180,180);
    private final IntegerValue mincustomYaw = new IntegerValue("MinCustomRotationYaw",180,-180,180) {
        @Override
        protected void onChanged(final Integer oldValue, final Integer newValue){
            final int i = maxcustomYaw.get();

            if (i < newValue)
                set(i);
        }
    };
    private final IntegerValue maxcustomPitch = new IntegerValue("MaxCustomRotationPitch",180,-180,180);
    private final IntegerValue mincustomPitch = new IntegerValue("MinCustomRotationPitch",180,-180,180) {
        @Override
        protected void onChanged(final Integer oldValue, final Integer newValue){
            final int i = maxcustomPitch.get();
    ...
    // Safety
    private final ListValue sameYValue = new ListValue("SameY", new String[] {"AutoJump","AutoSpeed","Off"},"Off");
...
//        counterY = 0.0;
    ...
    public void onUpdate(final UpdateEvent event) {
        ...
        if (counterY != animeY) {
            counterY = EaseUtils.easeOutExpo((System.currentTimeMillis() - startTime) / 1000)*animeY;
        }else if (EaseUtils.easeOutExpo((System.currentTimeMillis() - startTime) / 1000) > 1){
            counterY = animeY;
        }
    ...
    public void onMotion(final MotionEvent event) {
        ...
        // Lock Rotation
        if (rotationsValue.get().equals("Liquidbounce") && keepRotationValue.get() && lockRotation != null)
            RotationUtils.setTargetRotation(lockRotation);

        // Custom Rotation
        if (rotationsValue.get().equals("Custom") && keepRotationValue.get() && lockRotation != null)
            RotationUtils.setTargetRotation(customRotation);
    ...
    private void update() {
        ...
        if ((autoBlockValue.get() != "Off") ? InventoryUtils.findAutoBlockBlock() == -1 && !isHeldItemBlock : !isHeldItemBlock)
            return;
    ...
    private void findBlock(final boolean expand) {
        if (!mc.gameSettings.keyBindJump.isKeyDown()){
            switch (sameYValue.get()){
                ...
                case "AutoSpeed":{
                    samey = true;
                    LiquidBounce.moduleManager.getModule(Speed.class).setState(true);
                    break;
                }
                case "None":{
                    samey = false;
    ...
    private void place() {
        ...
        if (debug.get())
            debugText(this.getName(),"Player's posY:"+ (int) mc.thePlayer.posY + "LaunchY is:" + launchY + "Target yCoord is:" + targetPlace.getVec3().yCoord);

        if (!delayTimer.hasTimePassed(delay) || (samey && (((int) mc.thePlayer.posY - launchY) == 1)))
            return;
    ...
    public void onRender2D(final Render2DEvent event) {
        if (counterDisplayValue.get() == "Liquidbounce") {
        ...
        if (counterDisplayValue.get() == "String") {
            GlStateManager.pushMatrix();

            final String info = getBlocksAmount() + "blocks";
            final ScaledResolution scaledResolution = new ScaledResolution(mc);
            float scaledWidth = scaledResolution.getScaledWidth() / 2 - Fonts.font40.getStringWidth(info) / 2;
            float scaledHeight = scaledResolution.getScaledHeight() / 2 + 7;
            ...
            GlStateManager.resetColor();
            Fonts.fontRegular40.drawStringWithShadow(info, scaledWidth,
                    scaledHeight + (float) animeY - (float) counterY, Color.WHITE.getRGB());

            GL11.glTranslated(scaledWidth, scaledHeight + (float) animeY - (float) counterY,0.0);

            GlStateManager.popMatrix();
        }

        if (counterDisplayValue.get() == "Exhi") {
            GlStateManager.pushMatrix();
            ...
            final String info = getBlocksAmount() + "";
            final ScaledResolution scaledResolution = new ScaledResolution(mc);

            GlStateManager.resetColor();
            Fonts.minecraftFont.drawStringWithShadow(info, scaledResolution.getScaledWidth() / 2 - Fonts.font40.getStringWidth(info) / 2,
                    scaledResolution.getScaledHeight() / 2 + 7, Color.YELLOW.getRGB());
    ...
    private boolean search(final BlockPos blockPosition, final boolean checks) {
        ...
        if (rotationsValue.get().equals("Liquidbounce")) {
            RotationUtils.setTargetRotation(placeRotation.getRotation(), keepLengthValue.get());
            lockRotation = placeRotation.getRotation();
        }

        if (rotationsValue.get().equals("Custom")) {
            if (mc.thePlayer.movementInput.moveForward > 0)
                DiffYaw = RandomUtils.nextInt(mincustomYaw.get(),maxcustomYaw.get());
            if (mc.thePlayer.movementInput.moveForward < 0)
                DiffYaw = 0;
            customRotation = new Rotation(mc.thePlayer.rotationYaw + DiffYaw,RandomUtils.nextInt(mincustomPitch.get(),maxcustomPitch.get()));
            RotationUtils.setTargetRotation(customRotation, keepLengthValue.get());
        }
