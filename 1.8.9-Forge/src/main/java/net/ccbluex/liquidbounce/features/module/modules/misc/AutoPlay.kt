...
@ModuleInfo(name = "AutoPlay", description = "AutoPlay Server", category = ModuleCategory.MISC)
class AutoPlay : Module() {
            ...
            when (modeValue.get().toLowerCase()) {
                ...
                "blocksmc" -> {
                    if (clickState == 1 && text.contains("Only VIP players can join full servers!", true)) {
                        LiquidBounce.hud.addNotification(Notification(this.name, "Join failed! trying again...", NotifyType.WARNING,NotyIcon.WARNING,3000))
    ...
    private fun queueAutoPlay(runnable: () -> Unit) {
        ...
        if (this.state) {
            ...
            LiquidBounce.hud.addNotification(Notification(this.name, "Sending you to next game in ${delayValue.get()}s...", NotifyType.INFO, NotyIcon.INFO ,delayValue.get() * 1000))
        }
    }
