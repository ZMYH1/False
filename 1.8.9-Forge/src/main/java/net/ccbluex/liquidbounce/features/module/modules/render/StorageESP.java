...
public class StorageESP extends Module {
    ...
    private final IntegerValue R = new IntegerValue("Red",0,0,255);
    private final IntegerValue G = new IntegerValue("Green",66,0,255);
    private final IntegerValue B = new IntegerValue("Blue",255,0,255);
    private final IntegerValue alpha = new IntegerValue("Alpha",255,0,255);
        ...
        try {
            ...
            for (final TileEntity tileEntity : mc.theWorld.loadedTileEntityList) {
                ...
                if (chestValue.get() && tileEntity instanceof TileEntityChest && !ChestAura.INSTANCE.getClickedBlocks().contains(tileEntity.getPos()))
                    color = new Color(R.get(), G.get(), B.get(),alpha.get());
            ...
            for (final Entity entity : mc.theWorld.loadedEntityList)
                if (entity instanceof EntityMinecartChest) {
                    switch (mode.toLowerCase()) {
                        case "otherbox":
                        case "box":
                            RenderUtils.drawEntityBox(entity, new Color(R.get(), G.get(), B.get(),alpha.get()), !mode.equalsIgnoreCase("otherbox"));
                            break;
                        case "2d":
                            RenderUtils.draw2D(entity.getPosition(), new Color(R.get(), G.get(), B.get(),alpha.get()).getRGB(), Color.BLACK.getRGB());
                            break;
                        case "outline": {
                            final boolean entityShadow = mc.gameSettings.entityShadows;
                            mc.gameSettings.entityShadows = false;

                            RenderUtils.glColor(new Color(R.get(), G.get(), B.get(),alpha.get()));
                            OutlineUtils.renderOne(3F);
                            ...
                            OutlineUtils.renderThree();
                            mc.getRenderManager().renderEntityStatic(entity, mc.timer.renderPartialTicks, true);
                            OutlineUtils.renderFour(new Color(R.get(), G.get(), B.get(),alpha.get()));
                            mc.getRenderManager().renderEntityStatic(entity, mc.timer.renderPartialTicks, true);
                            OutlineUtils.renderFive();
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        shader.stopDraw(new Color(R.get(), G.get(), B.get(),alpha.get()), mode.equalsIgnoreCase("shaderglow") ? 2.5F : 1.5F, 1F);
    }
}
