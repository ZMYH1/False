...
@ModuleInfo(name = "DamageParticle", description = "DamageParticle",category = ModuleCategory.RENDER)
class DamageParticle : Module() {
    ...
    private val fonts = FontValue("Fonts", Fonts.minecraftFont)
    ...
    fun onRender3d(event: Render3DEvent) {
        synchronized(particles) {
            ...
            for (particle in particles) {
                ...
                fonts.get().drawStringWithShadow(
                    particle.str,
                    -(fonts.get().getStringWidth(particle.str) / 2).toFloat(),
                    -(fonts.get().FONT_HEIGHT - 1).toFloat(),
