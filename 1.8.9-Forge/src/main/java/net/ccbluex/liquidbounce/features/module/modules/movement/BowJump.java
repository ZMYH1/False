...
@ModuleInfo(name = "BowJump", description = "Allows you to jump further with auto bow shoot.", category = ModuleCategory.MOVEMENT)
public class BowJump extends Module {
    ...
    private final ListValue rect = new ListValue("Color",new String[] {"Normal","Fade"},"Fade");
    private final IntegerValue fadespeed = new IntegerValue("FadeSpeed",100,0,1000);
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        case 0:
            ...
            if (slot < 0 || !mc.thePlayer.inventory.hasItem(Items.arrow)) {
                debugText(this.getName(),"No arrows or bow found in your inventory!");
                forceDisable = true;
                bowState = 5;
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        switch (rect.get()){
            ...
            case "Fade":{
                RenderUtils.drawFadeRect(scaledRes.getScaledWidth() / 2F - 30F, scaledRes.getScaledHeight() / 2F + 26F, scaledRes.getScaledWidth() / 2F - 30F + width, scaledRes.getScaledHeight() / 2F + 28F, getStatusColor(),100);
                break;
            }
        }
