...
class AutoPot : Module() {
    ...
    private val speed = BoolValue("SpeedPotion",true)
    ...
    fun onMotion(motionEvent: MotionEvent) {
        ...
        when (motionEvent.eventState) {
            PRE -> {
                ...
                if (speed.get()){
                    val speedPH = findSpeedPotion(36,45)
                    if (speedPH != -1){
                        potion = speedPH
                ...
                val speedPI = findSpeedPotion(9,36)
                if (speedPI != -1&& InventoryUtils.hasSpaceHotbar()){
                    if (openInventoryValue.get() && mc.currentScreen !is GuiInventory)
                        return

                    val oI = mc.currentScreen !is GuiInventory && simulateInventory.get()
    ...
    private fun findSpeedPotion(startSlot: Int, endSlot: Int): Int {
        for (i in startSlot until endSlot) {
            val stack = mc.thePlayer.inventoryContainer.getSlot(i).stack
