...
public final class RenderUtils extends MinecraftInstance {
    ...
    public static void drawFadeRect(float x,float y,float x2,float y2,Color color,int cycle){

        double width = (x2 -x);
        for (int i =1; i <= (width);i++){
            drawRect(x+i-1 , y,x+i ,y2, Palette
                    .fade(new Color(color.getRed(), color.getGreen(), color.getBlue(), 255), i, cycle).getRGB());
        }
    }

    public static void drawFadeString(String text, Float x, Float y, Color color, int cycle, FontRenderer gameFontRenderer){

        int width = text.length();
        int width2 = gameFontRenderer.getStringWidth(" ");
        for (int i =1; i <= width;i += width2){
            gameFontRenderer.drawStringWithShadow(text,x+i-width2,y,Palette
                    .fade(new Color(color.getRed(),color.getGreen(),color.getBlue(),255),i,cycle).getRGB());
        }
    }
    ...
    public static void drawCircleRect(float paramXStart, float paramYStart, float paramXEnd, float paramYEnd, float radius, int color, boolean popPush) {
